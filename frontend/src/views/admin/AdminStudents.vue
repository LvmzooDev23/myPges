...
async function load(page = 1) {
  ...
  error.value = ''
  try {
    const { data } = await client.get('/admin/students', { params: { page } })
...
<template>
  <div class="space-y-6">
    <div>
      ...
      <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900">Étudiants</h1>
    </div>
    ...
    <UiCard v-if="!loading" padding>
      <DataTable>
        ...
        <template #body>
          <tr v-for="s in items" :key="s.id" class="border-b border-slate-100 hover:bg-slate-50/80">
            <td class="px-4 py-3 font-medium text-slate-900">{{ s.user?.name || '—' }}</td>
            <td class="px-4 py-3 text-slate-600">{{ s.user?.email || '—' }}</td>
            <td class="px-4 py-3 text-slate-600">{{ s.university || '—' }}</td>
            <td class="px-4 py-3 text-slate-600">{{ s.degree || '—' }}</td>
      ...
      <p v-if="!items.length" class="py-8 text-center text-sm text-slate-500">Aucun étudiant.</p>
